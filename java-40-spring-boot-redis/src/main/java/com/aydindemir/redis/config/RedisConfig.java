...
public class RedisConfig {
	...
	RedisTemplate<String, Object> redisTemplate(RedisConnectionFactory cf) {
		...
		RedisSerializer<Object> j = RedisSerializer.json();
		t.setKeySerializer(s);
		t.setHashKeySerializer(s);
		t.setValueSerializer(j);
		t.setHashValueSerializer(j);
		t.afterPropertiesSet();
		return t;
	}

	@Bean
	CacheManager cacheManager(RedisConnectionFactory cf, RedisLearningProperties p) {
		RedisCacheConfiguration c = RedisCacheConfiguration.defaultCacheConfig().entryTtl(p.getProductCacheTtl())
